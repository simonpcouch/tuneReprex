...
A package to help minimally reproduce failures seen on [tidymodels/finetune#50](https://github.com/tidymodels/finetune/pull/50).
...
For some reason, in the case of finetune (and seemingly only on some operating systems), finetunes's error handling infrastructure is tripped up by the deprecation warning in:
...
...fixes the issue. Switching to `tune::tune_grid` fixes the issue, which gives the usual "All models failed" warning.
